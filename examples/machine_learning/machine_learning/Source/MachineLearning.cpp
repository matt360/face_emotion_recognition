...
void MachineLearning::RecognizeEmotion()
{
    ...
    */
    ...
    unsigned long smallest_weighting_vector_size = std::min({ smileFeatures.weightingsVector.size(), pictureFeatures.weightingsVector.size(), smileFeatures.weightingsVector.size() } ); // use { initializer list } to get the mininum value from more than 2 variables

    const int smile_prob = CheckForEmotion(smallest_weighting_vector_size, smileFeatures, pictureFeatures);
    const int sad_prob = CheckForEmotion(smallest_weighting_vector_size, smileFeatures, pictureFeatures);
    const int angry_prob = 1;
